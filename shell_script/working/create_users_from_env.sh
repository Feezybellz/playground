...
create_mysql_user() {
    ...
    # Create the user
    mysql -u root -p -e "CREATE USER IF NOT EXISTS '$db_user'@'18.216.70.37' IDENTIFIED BY '$db_password';"
...
for config_file in /var/*/.env/config.php; do
    if [ -f "$config_file" ]; then
        db_name=$(grep -oP "putenv\('DB_NAME=\K.*?\);" "$config_file" | tr -d "\'")
        db_password=$(grep -oP "putenv\('DB_PASSWORD=\K.*?\);" "$config_file" | tr -d "\'")
        db_user=$(grep -oP "putenv\('DB_USER=\K.*?\);" "$config_file" | tr -d "\'")
