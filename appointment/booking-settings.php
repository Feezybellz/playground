...
//ALTER TABLE `read_businesses` ADD `booking_setting` TEXT NULL AFTER `payment_settings`;

$_GET['data'] = "Mq";


if (!isset($_GET['data']) || empty($_GET['data']) || ($_GET['data'] == "")) {

  header("Location:/dashboard");
}else{
  $businessId = base64_decode($_GET['data']);
}


// $business = selectContent($conn, "read_businesses", ['id' => $businessId,]);
...
// $bizId = base64url_decode($_GET['data']);
$bizId = base64_decode($_GET['data']);

if ($business[0]['booking_setting'] != NULL || !empty($business[0]['booking_setting']) || $business[0]['booking_setting'] !="") {
	
	if (@unserialize($business[0]['booking_setting'])) {
			$bookingSettingIsAvailable = true;
			$bookingSettingData = unserialize($business[0]['booking_setting']);
			$bookingURL = $business[0]['booking_url'];

			// die(var_dump($bookingSettingData));
	}
}


// $bookings = selectContent($conn, "read_booking_data", [ 'input_business_id' =>$bizId]);
$bookings = [['id'=>"1", 'hash_id'=>"94u3949_sburh8r", 'input_booking_date'=>"2023-03-12", 'input_name'=>"Afeez Bello",],];

$formattedEvents = [];


foreach ($bookings as $key => $value) {
  $formattedEvents[] = ['title'=>$value['input_name'], 'start'=>$value['input_booking_date'], 'link_to_url'=>"/booking-info/".$value['hash_id']."-".$value['id']];
}

$allEvents = json_encode($formattedEvents);

// $contacts = selectContent($conn, "panel_contact", ['input_user_id'=> $userId, 'input_business_id' =>$bizId]);

// $projects = selectContent($conn, "panel_project", ['user_id'=> $userId, 'business_id' =>$bizId]);

// $deals = selectContent($conn, "panel_deal", ['user_id'=> $userId, 'business_id' =>$bizId]);


// $contactByName = [];
// foreach ($contacts as $key => $value) {
//   $contactByName[$value['id']] = $value['input_contact_name'];
// }
// $jsonContacts = json_encode($contacts);
// $jsonProjects = json_encode($projects);
// $jsonDeals = json_encode($deals);
// var_dump($contacts);


// include APP_PATH.'/views/includes/db-header.php';
...
<!--end::Header-->
<head>
  <link rel="stylesheet" href="assets/css/booking-settings-updated.css">
</head>
<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
							...
							<div id="kt_content_container" class="container-xxl"><div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
								...
								<div class="card card-flush">
									...
									<div class="d-flex justify-content-end">
											...
											<div id="book_url_block" class="px-7 py-5 input-group mb-5 row" style="display: <?=isset($bookingURL) ? "" : "none"?>;">
												<div class="col-md-6 col-lg-6">
										    		<input type="text" class="form-control" id="book_url_input" value="https://<?=$_SERVER['HTTP_HOST']?>/book/<?=$bookingURL ?? ""?>" aria-describedby="basic-addon3" readonly/>
												</div>
												<div class="col-md-6 col-lg-4">
                            <button class="btn  btn-primary" onclick="copyText('book_url_input')" ><i class="fa fa-copy"></i>Copy Url </button>
												</div>
										  </div>
										...
										<div class="px-7 py-5">
	                          			...
	                          			<div class="d-flex align-items-center mb-5">
																	...
																	<!--end::Symbol-->
																	<!--begin::Title-->
																	<div class="d-flex flex-column" data-bs-toggle="modal" data-bs-target="#bookingSettingModal">
																		<a href="#" class="fs-6 text-gray-800 text-hover-primary fw-bold"><strong>Booking Settings</strong></a>
          ...
          <!-- Create Contact Modal Begin -->
      <div class="modal fade" id="bookingSettingModal" tabindex="-1" style="display: none;" aria-hidden="true">
			<!--begin::Modal dialog-->
				...
				<div class="modal-content">
					...
					<form class="form fv-plugins-bootstrap5 fv-plugins-framework" action="#" id="kt_modal_new_address_form">
							...
							<div class="scroll-y me-n7 pe-7" id="kt_modal_new_address_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_new_address_header" data-kt-scroll-wrappers="#kt_modal_new_address_scroll" data-kt-scroll-offset="300px" style="">
								...
								<!--begin::Input group-->
									...
									<div class="col-md-12 fv-row fv-plugins-icon-container">
										<!--begin::Label-->
										<label class="required fs-5 fw-bold mb-2">Create Booking Schedule</label>
										<!--end::Label-->
										<!--begin::Input-->

										<?php foreach ($daysOfWeekArr as $key => $value): ?>
											
										<div style="margin-top: 15px;" id="<?=$value?>Schedule" data-day="<?=$value?>" data-week-days="">
											
											<input type="checkbox" data-day="<?=$value?>" data-parent-id="<?=$value?>Schedule" onclick="checkDayFunc(this)" class="form-check-input"> <label style="font-weight:bold;"><?=$value;?></label>
											<br>

											<div id="timeSetup"  style="display:none; margin-top: 5px;">
												<label>Open</label> <input id="openTimeInput" type="time" placeholder="Select Time" class="form-control form-control-solid">

												<label>Close</label> <input id="closeTimeInput" type="time" placeholder="Select Time" class="form-control form-control-solid"> 
											</div>

											<div id="allDayChecker"  style="display:none; margin-top: 5px;">
												
											<input type="checkbox" data-day="<?=$value?>" data-parent-id="<?=$value?>Schedule" onclick="notAvailableAllDayFunc(this)" class="form-check-input"> <label>Not available all day</label>
											</div>
										...
										<!--begin::Col-->
											<br>
											<label for="basic-url" class="form-label">Booking URL</label>
											<div class="input-group mb-5">
											    <span class="input-group-text" id="basic-addon3">https://<?=$_SERVER['HTTP_HOST']?>/book/</span>
											    <input type="text" class="form-control" id="urlInput" value="<?=$bookingURL ?? ""?>" aria-describedby="basic-addon3"/>
											    <p>Special Characters Allowed are (- and _ ) only</p>
						...
						<div class="modal-footer flex-center">
							...
							<!--end::Button-->
							<!--begin::Button-->
							<button type="button" onclick="submitBookingScheduleSettingFunc(this)" data-form-id="bookingSettingModal" id="kt_modal_new_address_submit" class="btn btn-primary">
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
      ...
      eventClick: function(_e) {
          // event.prevenDefault();
          console.log(event)
          if (_e.event._def.extendedProps.link_to_url) {
              var eventUrl = _e.event._def.extendedProps.link_to_url;
              if (eventUrl != "") {
                _e.jsEvent.preventDefault()
    ...
    calendar.render();
									...
									weekDays = document.querySelectorAll('[data-week-days=""]');

									weekDays = [...weekDays];


									<?php if ($bookingSettingIsAvailable ?? false): ?>
										<?php $i = 0; foreach ($bookingSettingData as $key => $bookingSettingDataValue): $ii = $i++?>

											scheduleDays = weekDays[<?=$ii?>].dataset.day.toLowerCase();

											<?php if ($bookingSettingDataValue['availability']): ?>
												scheduleData[scheduleDays] = {availability:<?=json_encode($bookingSettingDataValue['availability'])?>};

												weekDays[<?=$ii?>].querySelector('[onclick="checkDayFunc(this)"]').click();

												<?php if (isset($bookingSettingDataValue['time'])): ?>
													
													weekDays[<?=$ii?>].querySelector('[onclick="notAvailableAllDayFunc(this)"]').click();
													weekDays[<?=$ii?>].querySelector("#openTimeInput").value = "<?=$bookingSettingDataValue['time']['open']?>";

													weekDays[<?=$ii?>].querySelector("#closeTimeInput").value = "<?=$bookingSettingDataValue['time']['close']?>";



												<?php endif ?>
												// weekDays[<?=$ii?>].querySelector('[onclick="notAvailableAllDayFunc(this)"]').checked = true;
											<?php else:?>
												scheduleData[scheduleDays] = {availability:<?=json_encode($bookingSettingDataValue['availability'])?>};
									...
									for (var i = 0; i<weekDays.length; i++) {
										scheduleDays = weekDays[i].dataset.day.toLowerCase();

										scheduleData[scheduleDays] = {availability:false};
									}
									
									<?php endif ?>

									function checkDayFunc(e){
										var parentId = e.dataset.parentId;
										var parentElem = document.getElementById(parentId);
										var allDayChecker = parentElem.querySelector("#allDayChecker")
										var timeSetup = parentElem.querySelector("#timeSetup")

										var scheduleDays = (e.dataset.day).toLowerCase();

										var allDayCheckerCheckBox = parentElem.querySelector('[onclick="notAvailableAllDayFunc(this)"]');

										// console.log(allDayCheckerCheckBox)
										
										if(e.checked){
											// alert("checked");
											allDayChecker.style.display = "";

											scheduleData[scheduleDays].availability = true;
											...
											if(allDayCheckerCheckBox.checked){
												allDayCheckerCheckBox.click();
											}
											...
											allDayChecker.style.display = "none";
									...
									function notAvailableAllDayFunc(e){
										var parentId = e.dataset.parentId;
										var parentElem = document.getElementById(parentId);
										var timeSetup = parentElem.querySelector("#timeSetup")
										...
										if(e.checked){
											// alert("checked");
											timeSetup.style.display = "";

											scheduleData[scheduleDays].time = {};



										}else{
											delete scheduleData[scheduleDays].time;
											timeSetup.style.display = "none";
									...
									function submitBookingScheduleSettingFunc(e){

										errorOnSubmit = [];
										var availabilityInAWeek = false;
										var bookingUrl = urlInput.value;
										// console.log(weekDays)

										for (var i = 0; i<weekDays.length; i++) {
											scheduleDays = weekDays[i].dataset.day.toLowerCase();
											openTime = weekDays[i].querySelector("#openTimeInput").value;
											closeTime = weekDays[i].querySelector("#closeTimeInput").value;

											allDayCheckerCheckBox = weekDays[i].querySelector('[onclick="notAvailableAllDayFunc(this)"]')

											// alert(allDayCheckerCheckBox)
											// console.log(day)

											if(scheduleData[scheduleDays].availability === true){
												//Check if the business is at least available the
												availabilityInAWeek = true;


												if(allDayCheckerCheckBox.checked === true){
													scheduleData[scheduleDays].time = {open:openTime, close:closeTime};
												}else{
													scheduleData[scheduleDays].time = {open:"00:00", close:"23:59"};

												}

											}else{
												if (scheduleData[scheduleDays].hasOwnProperty("time")) {
														delete scheduleData[scheduleDays].time;
										...
										if(bookingUrl.length< 1 || bookingUrl == ""){
											alert("Please input URL")
											errorOnSubmit.push(true)
										}else if(!urlPattern.test(bookingUrl)){
											alert("Invalid URL Parameter")
											errorOnSubmit.push(true)
										}

										if(availabilityInAWeek === false){
										...
										if (errorOnSubmit.length < 1) {

											var url = "/booking-settings-backend"
											var data = {
												bizId,
												scheduleData,
												bookingUrl,
        								...
        								if (returnData['success']) {
        										book_url_block.style.display = "";
        										book_url_input.value = "https://<?=$_SERVER['HTTP_HOST']?>/book/"+bookingUrl
								          // setTimeout(function(){
								            Swal.fire(
								            'Success',
								            returnData.success,
		...
		const groupBy = (arr, fn) =>
          ...
          createYear = generate_year_range(1970, 2060);
          /** or
          ...
          */
          ...
          function showCalendar(month, year) {
          	...
          	// alert(monthFormatted+"| "+year);

          	var dateToFetchStart = `${year}-${monthFormatted}-01`;
          	var dateToFetchEnd = `${year}-${monthFormatted}-31`;


          	var bizId = "<?=$_GET['data']?>";
    				var url = "/fetch-booking-backend"
          	...
          	var data = {
				    ...
				    ajaxPost(url,data, (err,res) => {
				      ...
				      if (returnData.success) {
				      	// returnData.success.map((dataDate) => {
				      	// 	alert(+dataDate.taskDayStart)
				      	// })

				      	var bookingDataDates = groupBy(returnData.success, dataDate => dataDate.bookingDay);

				      	console.log(bookingDataDates)
                ...
                for ( var j = 0; j < 7; j++ ) {
                    if ( i === 0 && j < firstDay ) {
                    	...
                    	// alert(date);
                    		...
                    		if (date.toString().length < 2) {var stringDate = "0"+date}else{var stringDate = date}

                    			var thisDateBookingData = bookingDataDates[stringDate];
                    		console.log(bookingDataDates[stringDate])
                    		// console.log(date.length, stringDate,taskArrStart, taskArrEnd);


                    		// htmlTaskStart = dateTasksStart.map( (dateData) => {
                    		// 	dateData.input_title;
                    		// })
                        ...
                        cell.innerHTML = `<span style="font-size:20px; font-weight: bold;" data-booking-dates='${JSON.stringify(thisDateBookingData)}'>${date} <p id="detail"></p></span>`;
          ...
          function displayBookingDateFunc(e){

          	var taskStartsHtml = "";
          	var taskEndsHtml = "";

          	if (e.dataset.bookingDates != "undefined") {

          	var bookingDates = JSON.parse(e.dataset.bookingDates);

          	e.querySelector("#detail").innerHTML += bookingDates.map((data) => {
          		// alert(taskData.input_title)
          		return `<span class="btn btn-primary" style="font-size:7px; width:100%; height:auto; background-color:blue;">${data.input_name}</span>`
          	})
          	}
          ...
          function calendarNoter(){
          	var bookingDataDatesColumn = document.querySelectorAll("[data-booking-dates]");
          	bookingDataDatesColumn = [...bookingDataDatesColumn];

          	console.log(bookingDataDatesColumn)

          	bookingDataDatesColumn.map((e) => {
          			displayBookingDateFunc(e)
          		// if (e.dataset.taskStarts != "undefined") {
          		// 		displ
          ...
          function copyText(elem) {
            // Get the text field
            var textElement = document.getElementById(elem);
          ...
          // window.onload = () => calendarNoter();

          </script>



<div class="footer py-4 d-flex flex-lg-column" id="kt_footer" style="position: fixed; bottom: 0px; width: 100%;">
  <!--begin::Container-->
  <div class="container-fluid d-flex flex-column flex-md-row align-items-center justify-content-between">
    <!--begin::Copyright-->
    <div class="text-dark order-2 order-md-1">
      <span class="text-muted fw-bold me-1"><?php echo date("Y") ?> &copy; </span>
      <a href="/" target="_blank" class="text-gray-800 text-hover-primary"><?php echo $site_name ?></a>
    </div>
    <!--end::Copyright-->
    <!--begin::Menu-->
...
<script type="text/javascript">
  ...
  <?php if (count($uri) > 1 && $uri[1] == "crm"): ?>
    window.onload = () =>{ crmSubMenu.click()}
  <?php endif ?>

  <?php if (count($uri) > 1 && $uri[1] == "social-media"): ?>
    window.onload = () =>{ socialMediaSubMenu.click()}
  <?php endif ?>
</script>
<!--begin::Javascript-->
...
<!-- Mirrored from preview.keenthemes.com/metronic8/demo1/dark/index.html by HTTrack Website Copier/3.x [XR&CO'2014], Mon, 01 Nov 2021 14:35:35 GMT -->
</html>
