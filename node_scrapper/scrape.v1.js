...
(async () => {
  ...
  const sanitizeFilePath = (urlPath) => {
    ...
    // Ensure the file has a .html extension if it doesn't already
  ...
  const saveAsset = async (response) => {
    ...
    let filePath = sanitizeFilePath(url.pathname);
  ...
  const crawlPage = async (pageUrl) => {
    ...
    let filePath = sanitizeFilePath(urlObj.pathname);
    if (filePath.endsWith("/")) {
      filePath = path.join(filePath, "index.html");
    }
